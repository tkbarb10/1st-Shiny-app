...
saveRDS(employ_data, file = "employment_data.rds")

data <- readRDS("employment_data.rds")


data_g <- readRDS("graduates_by_institutions.rds")
...
server <- function(session, input, output) {
  ...
  #Panel: Main > Summary > Tables and Pie Chart
  
  #Summary Section
  
  output$datahead <- renderUI({
    table <- data %>%
      ...
      #summarise_at(.vars = names(.)[5], list(min, max)) %>%
      kable(
        "html",
        col.names = c(
          "University",
  ...
  output$densityPlot <- renderPlotly({
    ...
    ggplotly(
      ggplot(data = dent(), aes(x = .data[[input$selectVar]])) +
        geom_density(aes(fill = university), linewidth = 1, alpha = .75) +
  ...
  output$detailTable <- renderUI({
    ...
    table <- isolate({
      detailTB() |> 
        data.frame() |> 
        kable("html",
              col.names = c("Year", "Median Monthly Income",
                            "Fulltime Employment Rate")) |> 
